...
function Participant(name: string) {
  ...
  this.send = function (message: string, to: participantI) {
    this.chatroom.sendMessage(message, this, to);
  };
  this.receive = function (message: string, from: participantI) {
...
function Chatroom() {
  ...
  this.register = function (...args: Array<participantI>) {
  ...
  this.unregister = function (p: participantI) {
    delete this.participants[p.name];
    p.chatroom = null;
  };

  this.sendMessage = function (...args: Array<participantI>) {
    ...
    if (to) {
      ...
      for (let k in this.participants) {
        if (k !== from.name) {
